...
model = (BartEncodecForConditionalGeneration
           .from_pretrained("voidful/bart-base-unit"))
        ...
        for i in range(8):
            encodec_input.append(
                tokenizer.convert_tokens_to_ids(
                    [f"v_tok_{u + i * 1000}" 
                     for u in batch[f'encodec_{i}'][b]]))
        input_datas.append(encodec_input)
        
    import torch
    tensors = [torch.tensor(data).transpose(0, 1) for data in input_datas]
    padded_input_datas = torch.nn.utils.rnn.pad_sequence(
        tensors, batch_first=True, padding_value=tokenizer.pad_token_id)  # pad 到最長
    padded_input_datas = padded_input_datas[:, :max_len].transpose(1, 2)  # [BSZ, 8, MAX_SEQ] 切到
    attention_masks = torch.zeros((len(input_datas), max_len), dtype=int)
    for data_id, data in enumerate(input_datas):
        attention_masks[data_id, :len(data[0])] = 1
